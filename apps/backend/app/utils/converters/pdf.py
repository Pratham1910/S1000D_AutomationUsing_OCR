...
class PDFConverter(BaseConverter):
        ...
        try:
            ...
            for page_num in range(page_count):
                ...
                if page_num == 0:
                    ...
                    logger.info(f"First page image size: {pix.width}x{pix.height} (DPI: {dpi})")
                else:
                    # 检查当前页面转换后的尺寸是否与首页一致
                    if pix.width != target_width or pix.height != target_height:
                        logger.warning(
                            f"Page {page_num + 1} size ({pix.width}x{pix.height}) does not match first page ({target_width}x{target_height})"
                        )
                        ...
                        logger.info(
                            f"Resized page {page_num + 1} to unified size: {target_width}x{target_height}"
                        )
            ...
            raise ConversionFailedError(f"PDF conversion failed: {str(e)}") from e
    ...
    async def validate(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"PDF file validation passed, total pages: {page_count}")
            return True

        except Exception as e:
            logger.error(f"PDF file validation failed: {str(e)}")
            return False
