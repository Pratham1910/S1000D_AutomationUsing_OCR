...
# It now handles both strict 11-part DMCs and condensed 8-part DMCs.
    ...
    def convert_olist(node)
      ...
      node.items.each_with_index do |item, idx|
        ...
        # item.content handles text and nested blocks correctly for list items
        inner_content = if item.blocks?
                         item.content
                       elsif item.text && !item.text.strip.empty?
                         "<para>#{esc_text(item.text.strip)}</para>"
                       else
                         "<para/>" # S1000D listItems usually require some content
                       end
      ...
      # Common S1000D practice is to wrap lists in <para> when they are flow content
   ...
   # UPDATED convert_ulist with the global helper logic
    ...
    def convert_ulist(node)
      # NEW: Check if this ulist is being used as a special reference block.
      # A reference block is a ulist with attributes and exactly one simple item.
      if (node.attr?('dmref') || node.attr?('dmc') || node.attr?('pmref') || node.attr?('pmc')) &&
        ...
        # It matches our pattern! Delegate to the global helper.
        return generate_global_ref(node) || ""
      end

      # --- The original, generic ulist logic is the fallback ---
...
def convert_section(node)
  ...
  output_parts.join("\n")
    ...
    def convert_inline_anchor(node)
      ...
      when :link
        target = node.target
        # Remove a potential .xml file extension to isolate the DMC string
        potential_dmc = target.gsub(/\.html$/i, '')
        
        # Use your existing helper function to try and parse the string
        parsed_dmc = parse_dmc_string(potential_dmc)

        if parsed_dmc
          # SUCCESS: The link target is a valid DMC.
          # Build the desired <dmRef> structure.
          # The attributes are created by mapping the key-value pairs from the parsed hash.
          dm_code_attrs_xml = parsed_dmc.map { |k, v| %(#{k}="#{esc_text(v)}") }.join(' ')
          
          # The link text (e.g., [Link]) is ignored to match your required output.
          <<~XML.strip
          <dmRef>
            <dmRefIdent>
          ...
          # FAILURE: The link is not a DMC, so treat it as a standard external link.
  ...
  [:head, :foot, :body].each do |tsec_name|
    ...
    rows_collection.each do |row|
      ...
      row.each do |cell|
        ...
        if (halign = cell.attr('halign'))
          entry_attrs << %( align="#{halign}")
        end
        if (valign = cell.attr('valign'))
          entry_attrs << %( valign="#{valign}")
        end

        # --- Handle CALS column span (namest/nameend) ---
        col_start = cell.column.attr('colnumber').to_i
        col_span = cell.colspan.to_i
        if col_span > 1
          col_end = col_start + col_span - 1
          entry_attrs << %( namest="c#{col_start}" nameend="c#{col_end}")
        else
          entry_attrs << %( namest="c#{col_start}" nameend="c#{col_start}")
        end

        # --- Handle CALS row span (morerows) ---
        row_span = cell.rowspan.to_i
        entry_attrs << %( morerows="#{row_span - 1}") if row_span > 1
        ...
        cell_body = case cell.style
                    ...
                    when :literal
                      ...
                      text.empty? ? "<para/>" : "<para>#{esc_text(text)}</para>"
                    end
