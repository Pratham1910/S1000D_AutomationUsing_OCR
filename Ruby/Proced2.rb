...
module Asciidoctor
  class Converter::S1000D < Converter::Base
    ...
    # ***** REVISED generate_req_cond_group_xml *****
        # ***** REVISED generate_req_cond_group_xml (Pure Text Parsing for Attributes) *****
        # ***** REVISED generate_req_cond_group_xml (Simplified approach) *****
        # ***** REVISED generate_req_cond_group_xml (Final Cleanup Adjustment) *****
      ...
      section_node.blocks.each_with_index do |block, idx|
        ...
        when :ulist, :olist
          ...
          block.items.each do |item|
            # Standard attributes parsed by Asciidoctor
            item_id_val = item.id # Get ID directly
            item_applic_ref_val = item.attr('applic_ref') # Get applic_ref attribute
            ...
            # Try to get pmref/dmref as regular Asciidoctor attributes first
            ...
            text_for_condition = raw_item_text_content.dup.strip # Start with stripped text

            if pm_ref_attr && !pm_ref_attr.strip.empty?
              pm_to_parse = pm_ref_attr.strip
              # Clean the literal text if pmref was an attribute but also appeared in text
              ...
              # If not found as attributes, look for patterns in the text
            ...
            # Final cleanup: If item.id or item.attr('applic_ref') were nil,
            # it means Asciidoctor didn't parse them from the primary attribute block.
            # Try to remove any literal `[id=...]` or `[applic_ref=...]` if they appear at the start of the text.
            # This handles the case where the *entire* `[...]` block was treated as text.
            
            # If item_id_val was nil (Asciidoctor didn't parse an ID) AND text starts with [id=...
            if item_id_val.nil? && text_for_condition.match?(/\A\s*\[id=([^,\]\s]+)/i)
              text_for_condition.sub!(/\A\s*\[id=[^\]]+\]\s*/i, '').strip!
            end

            # If item_applic_ref_val was nil (Asciidoctor didn't parse applic_ref) AND text starts with [applic_ref=...
            # or [..., applic_ref=...]
            if item_applic_ref_val.nil?
              # More robustly try to remove a leading block if it contains applic_ref and wasn't parsed
              if text_for_condition.match?(/\A\s*\[(?:[^\]]*?,?\s*)?applic_ref=([^,\]\s]+)/i)
                 text_for_condition.sub!(/\A\s*\[[^\]]+\]\s*/i, '').strip! # Remove the whole leading [...] block
              end
            end
            
            # A very generic final cleanup if text still starts with an unparsed attribute block
    ...
    def determine_internal_ref_target_type(target_node, target_id_for_warning)
      ...
      when :olist_item, :ulist_item, :list_item
        (target_node.role == 'proceduralStep' || target_node.id&.start_with?('step_')) ? ' internalRefTargetType="irtt0F"' : ' internalRefTargetType="irtt04"'
      when :section
    ...
    def generate_req_safety_xml(section_node)
      ...
      if safety_elements.empty? && !section_node.blocks.any? { |b| b.context == :paragraph && b.source.downcase.include?("no safety condition") }
        ...
        # If section was present but empty, it could mean "no safety", or it was authored incorrectly.
        # Let's default to noSafety if elements are empty, unless specific content dictates otherwise.
      end
    ...
    def generate_preliminary_requirements_xml(document_node)
     ...
     def generate_main_procedure_steps_xml(document_node)
      ...
      blocks_to_consider.each_with_index do |block, index|
        # Skip simple paragraphs that are likely just xref targets or metadata
        if block.context == :paragraph && block.blocks.empty? && block.id && !block.title?
            ...
            is_short_xref_target_text = block.source.strip.length < 70 && # Increased length slightly
                                        (block.source.downcase.include?(block.id.downcase) ||
                                         block.source.match?(/refer to|target for/i)) # Common phrases for xref targets
            is_just_icn_para = block.source.match?(/^ICN-[A-Z0-9\-]+$/i) # Paragraph that is ONLY an ICN

            if is_dmc_like || is_short_xref_target_text || is_just_icn_para
              next # Skip this block from becoming a proceduralStep
            end
        end

        if block.context == :olist
          # Each top-level item of this olist becomes a proceduralStep
          block.items.each_with_index do |li, li_idx|
            step_id = li.id # Prefer explicit ID from [[step_id]]
            # If no ID on li, generate one based on olist ID or default
            step_id ||= "#{block.id || 'procstep'}" 
            ...
            # li.content will recursively call converters for blocks within this list item.
            # If li.content contains another olist, the revised convert_olist will handle it.
            inner_content_xml = li.content 
            procedural_steps_xml << "<proceduralStep#{step_attrs}>#{title_xml}#{inner_content_xml}</proceduralStep>"
          end
        elsif block.context == :section && block.role != 'attribute_def' # Sections can be steps, unless they are special defs
        ...
        # Handle other block types that can be direct procedural steps (e.g., a figure, a paragraph that is a step)
        elsif [:image, :listing, :literal, :admonition, :table, :open, :paragraph].include?(block.context)
          # Ensure this block isn't a definition block processed elsewhere
          ...
          unless is_empty_para_without_id
            ...
            # If the block itself has a title (e.g., figure with title), it's part of converted_content.
            # A proceduralStep can also have its own title. If block.title is relevant for the step itself:
            title_outer_xml = ""
            if block.title? && !converted_content.strip.start_with?("<title>") && !converted_content.strip.match?(/<figure.*<title>/m)
                 # Example: A paragraph block with a title `.Step Title` that becomes a step.
                 # The convert_paragraph doesn't add <title> itself.
                 # This case is more for blocks like :open or :paragraph that have a title attribute.
                 # An :image block's title becomes <figure><title>...
                 # An :olist item's title is handled above.
                 # This primarily applies if a general block is treated as a step and has an Asciidoctor title.
                 is_already_titled_in_content = case block.context
                                                when :image, :listing, :table # these converters handle their own titles within their structure
                                                   true
                                                else
                                                   false
                                                end
                 title_outer_xml = "<title>#{esc_text(block.title)}</title>" unless is_already_titled_in_content
            end
            procedural_steps_xml << "<proceduralStep#{step_attrs}>#{title_outer_xml}#{converted_content}</proceduralStep>"
          end
        end # end of block type checks
      end # end of blocks_to_consider.each

      steps_content = procedural_steps_xml.join("\n")
      # S1000D procedural content usually requires at least one step, or specific <noProcedure> etc.
      # For mainProcedure, it expects one or more proceduralSteps.
    ...
    def generate_fault_isolation_main_procedure_xml(document_node)
      ...
      blocks_to_process = if fault_main_section
                            ...
                            document_node.blocks.reject do |b|
                              # Define conditions for skipping blocks if no main fault section
    ...
    def generate_flat_global_asserts_xml(doc_attrs)
      ...
      result_xml.gsub(/^/, '      ') # Indent for placement in <applic>
    end
    ...
    def build_ident_and_status_section_xml(doc_attrs, dm_code_attrs_in, act_dm_ref_for_dmstatus, global_applic_text_val, brex_dm_code_attrs_in, rfu_elements_xml, current_node_for_title)
        ...
        if @global_applic_eval_hash && @global_applic_eval_hash.is_a?(Hash) && !@global_applic_eval_hash.empty?
            raw_xml = build_applic_condition_xml(@global_applic_eval_hash)
            global_applic_xml = raw_xml.strip.gsub(/^/, '      ') unless raw_xml.strip.empty?
        else
            global_applic_xml = generate_flat_global_asserts_xml(doc_attrs)
            # Warnings for empty/failed global applic if relevant attributes/blocks were present
            if global_applic_xml.empty?
                if @global_applic_eval_hash&.empty?
                     warn "asciidoctor: INFO: 'global_applicability_definition' block empty. No global conditions."
                elsif node_has_global_applic_block(doc_attrs[:document_node]) && @global_applic_eval_hash.nil?
                     warn "asciidoctor: INFO: 'global_applicability_definition' parse failed/empty. No global conditions."
                end
            end
        end
        ...
        <identAndStatusSection>
          ...
          <dmStatus issueType="new">
            ...
            <originator enterpriseCode="#{esc_text(enterprise_orig_code)}"><enterpriseName>#{esc_text(orig_name)}</enterpriseName></originator>
            #{act_dm_ref_for_dmstatus.empty? ? '' : act_dm_ref_for_dmstatus.strip.gsub(/^/, '    ')}
            <applic>
              <displayText><simplePara>#{esc_text(global_applic_text_val)}</simplePara></displayText>
            #{global_applic_xml.empty? ? '' : "\n" + global_applic_xml}
            </applic>
    ...
    def convert_document node
      ...
      node.blocks.each(&:convert) # First pass for definitions

      pal_xml = @s1000d_product_attribute_definitions.empty? ? "" : "<productAttributeList>\n#{@s1000d_product_attribute_definitions.map { |pa| "  #{pa}" }.join("\n")}\n</productAttributeList>"
      ...
      if dm_type == 'act'
        ...
        if pct_dmc_str && !pct_dmc_str.strip.empty?
          if pct_dm_c = parse_dmc_string(pct_dmc_str)
            ...
            warn "asciidoctor: INFO: dm-type 'act' expects 'pct-dmc' attribute."
        end
      end

      internal_pct_xml = @s1000d_product_definitions.empty? ? "" : "<productCrossRefTable>\n#{@s1000d_product_definitions.map { |p| "  #{p}" }.join("\n")}\n</productCrossRefTable>"
      ...
      if dm_type == 'act'
        ...
        act_outer_xml = act_children.empty? ? (warn "asciidoctor: INFO: dm-type 'act', no content for <applicCrossRefTable>." ; "") : "<applicCrossRefTable>\n#{act_children.map{|c| "  #{c}"}.join("\n")}\n</applicCrossRefTable>"
      end
      
      rag_xml = @s1000d_applic_definitions.empty? ? "" : "<referencedApplicGroup>\n#{@s1000d_applic_definitions.map { |a| "  #{a}" }.join("\n")}\n</referencedApplicGroup>"
      ...
      general_content_processed = node.blocks.map do |block|
        ...
        is_def_block ? nil : block.convert
      ...
      main_dm_content = case dm_type
                        when 'procedure', 'procedural'
                          ...
                          main_proc = generate_main_procedure_steps_xml(node)
                          closeout = generate_close_requirements_xml(node)
                          "<procedure>\n#{prelim}\n<mainProcedure>\n#{main_proc}\n</mainProcedure>\n#{closeout}\n</procedure>"
                        when 'fault', 'faultisolation'
                          fault_descr = general_content_processed.empty? ? "<faultDescr><para>No fault description provided.</para></faultDescr>" : "<faultDescr>\n  #{general_content_processed.gsub(/^/, '  ')}\n</faultDescr>"
                          prelim = generate_preliminary_requirements_xml(node)
                          fault_iso = generate_fault_isolation_main_procedure_xml(node)
                          closeout = generate_close_requirements_xml(node)
                          "<fault>\n#{fault_descr}\n<faultIsolation><faultIsolationProcedure><isolationProcedure>\n#{prelim}\n#{fault_iso}\n#{closeout}\n</isolationProcedure></faultIsolationProcedure></faultIsolation></fault>"
                        when 'act'
                          desc = general_content_processed.empty? ? "" : "<description>\n  #{general_content_processed.gsub(/^/, '  ')}\n</description>\n"
                          desc + act_outer_xml
                        when 'pct'
                          desc = general_content_processed.empty? ? "" : "<description>\n  #{general_content_processed.gsub(/^/, '  ')}\n</description>\n"
                          desc + internal_pct_xml
      ...
      final_content_parts = []
      final_content_parts << rag_xml.strip if !rag_xml.empty? && !['act', 'pct'].include?(dm_type)
    ...
    def convert_paragraph(node)
      ...
      if content.nil? || (content.is_a?(String) && content.strip.empty? && !node.id?)
         return %(<para#{applic_ref_attribute(node)} />) if node.id?
         return %(<para#{applic_ref_attribute(node)} />)
      end
      %(<para#{applic_ref_attribute(node)}>#{content}</para>)
    end
    ...
    def convert_open(node)
      ...
      if node.style == 'example' || node.role == 'example'
        ...
        return %(<levelledPara#{common_attributes(node.id)}#{applic_ref_attribute(node)} class="example">#{title_el}#{content}</levelledPara>)
      end
      node.title? ? %(<levelledPara#{common_attributes(node.id)}#{applic_ref_attribute(node)}><title>#{esc_text(node.title)}</title>#{content}</levelledPara>) : content
    end

    def convert_ulist(node)
      return process_as_product_attribute_definition(node) || "" if node.role == 'attribute_def'
      ...
      items = node.items.map do |item|
        ...
        inner = item.blocks? ? item.content : "<para#{applic_ref_attribute(item)}>#{esc_text(item.text)}</para>"
        "<listItem>#{inner}</listItem>"
      end.join
      "<randomList>#{items}</randomList>"
    end

     # In your S1000D Converter class

    # REVISED convert_olist (to wrap sequentialList in <para>)
    def convert_olist(node)
      item_outputs = [] # Stores { type: :proceduralStep or :listItem, xml: "..." }

      node.items.each_with_index do |item, idx|
        # Heuristic: an item becomes a <proceduralStep> if it has a title (item.text)
        # AND it contains further blocks (item.blocks?). This suggests a sub-procedure.
        is_sub_procedure_candidate = item.text && !item.text.strip.empty? && item.blocks?

        if is_sub_procedure_candidate
          step_id = item.id || "#{node.id || 'subproc'}-#{idx + 1}"
          step_attrs = common_attributes(step_id) + applic_ref_attribute(item)
          title_xml = "<title>#{esc_text(item.text.strip)}</title>"
          inner_content_xml = item.content # This will recursively call converters
          inner_content_xml = "<para/>" if inner_content_xml.strip.empty? # Ensure content
          item_outputs << { type: :proceduralStep, xml: "<proceduralStep#{step_attrs}>#{title_xml}#{inner_content_xml}</proceduralStep>" }
        else
          # This item is a regular list item for a <sequentialList>
          li_attrs = common_attributes(item.id) + applic_ref_attribute(item)
          li_content_parts = []
          if item.text && !item.text.strip.empty?
            # Item's principal text goes into a <para>
            li_content_parts << "<para>#{esc_text(item.text.strip)}</para>"
          end
          if item.blocks?
            li_content_parts << item.content # Add content from any nested blocks
          end

          final_li_content = li_content_parts.join("\n")
          final_li_content = "<para/>" if final_li_content.strip.empty? # Ensure listItem has valid S1000D content

          item_outputs << { type: :listItem, xml: "<listItem#{li_attrs}>#{final_li_content}</listItem>" }
        end
      end

      return "" if item_outputs.empty?

      # Determine if this olist should be a sequence of <proceduralStep>s or a <sequentialList>
      all_are_steps = item_outputs.all? { |out| out[:type] == :proceduralStep }
      all_are_list_items = item_outputs.all? { |out| out[:type] == :listItem }

      if all_are_steps
        # This olist resolved to a sequence of <proceduralStep>s directly
        item_outputs.map { |out| out[:xml] }.join("\n")
      elsif all_are_list_items
        # This olist resolved to items for a <sequentialList>
        # Attributes from the Asciidoctor list node (node.id, node.attr('applic_ref'))
        # should apply to the <sequentialList> element itself.
        list_attrs_for_seq_list = common_attributes(node.id) + applic_ref_attribute(node)
        list_item_xml = item_outputs.map { |out| out[:xml] }.join("\n")
        # Wrap the <sequentialList> in a <para>. The <para> itself is anonymous here.
        "<para><sequentialList#{list_attrs_for_seq_list}>#{list_item_xml}</sequentialList></para>"
      else
        # Mixed content: S1000D does not allow direct mixing of <proceduralStep> and <listItem>
        # as siblings under the same parent in most common structures.
        warn "asciidoctor: WARNING (convert_olist): Mixed item types (proceduralStep and listItem) in olist with ID '#{node.id || 'anonymous'}'. This will likely lead to invalid S1000D. Outputting as a raw sequence; validation will fail."
        item_outputs.map { |out| out[:xml] }.join("\n") # Output raw sequence, likely invalid
      end
    end

    # REVISED convert_ulist (to wrap randomList in <para>)
    def convert_ulist(node)
      # If this ulist is part of an attribute_def, it's handled by process_as_product_attribute_definition
      return "" if node.role == 'attribute_def' || node.style == 'attribute_def'

      # Attributes from the Asciidoctor list node for the <randomList>
      list_attrs_for_rand_list = common_attributes(node.id) + applic_ref_attribute(node)
      items = node.items.map do |item|
        item_attrs = common_attributes(item.id) + applic_ref_attribute(item) # Attributes on the <listItem>
        inner_content = if item.blocks?
                         item.content
                       elsif item.text && !item.text.strip.empty?
                         "<para>#{esc_text(item.text)}</para>" # No applic_ref on this inner para from item
                       else
                         "<para/>" # S1000D requires listItem to have content
                       end
        "<listItem#{item_attrs}>#{inner_content}</listItem>"
      end.join("\n")
      # Wrap the <randomList> in a <para>. The <para> itself is anonymous here.
    ...
    def convert_dlist(node)
      ...
      items = node.items.map do |terms, dd|
        term_xml = terms.map { |dt| esc_text(dt.text) }.join("; ") # Simplified term handling
        dd_content = if dd
                       dd.blocks? ? dd.content : (dd.text? ? "<para#{applic_ref_attribute(dd)}>#{esc_text(dd.text)}</para>" : "<para/>")
                     else
                       "<para/>"
                     end
      ...
      if node.role == 'global_applicability_definition' || node.style == 'global_applicability_definition'
        ...
        begin
          ...
          if parsed.is_a?(Hash) && !parsed.empty?
            ...
            warn "asciidoctor: WARNING: JSON in global_applicability_definition not a non-empty Hash."
            @global_applic_eval_hash = nil
          end
        rescue JSON::ParserError => e
          warn "asciidoctor: WARNING: JSON parse error in global_applicability_definition: #{e.message}"
          @global_applic_eval_hash = nil
        end
        return ""
      end
    ...
    def convert_section(node)
      attrs = common_attributes(node.id) + applic_ref_attribute(node)
      title = node.title? ? esc_text(node.title) : "Untitled Section"
      "<levelledPara#{attrs}><title>#{title}</title>#{node.content}</levelledPara>"
    end
    ...
    def convert_inline_anchor(node)
      ...
      when :xref
        ...
        type_attr = determine_internal_ref_target_type(target_node, target)
        display = node.text || target
        "<internalRef internalRefId=\"#{esc_text(target)}\"#{type_attr}>#{esc_text(display)}</internalRef>"
      when :link
        display = node.text || node.target
        "<externalRef destination=\"#{esc_text(node.target)}\">#{esc_text(display)}</externalRef>"
      when :ref, :bibref
        warn "asciidoctor: WARNING: Unsupported anchor type '#{node.type}'. Outputting as text."
        esc_text(node.text || node.target)
      else
        warn "asciidoctor: WARNING: Unknown anchor type '#{node.type}'. Outputting as text."
        esc_text(node.text || node.target)
      end
    end
    ...
    def convert_table(node)
      attrs = common_attributes(node.id) + applic_ref_attribute(node)
      pgwide = (node.option?('pgwide') || node.attr?('width', '100%')) ? 1 : 0
      s1k_frame = case node.attr('frame', 'all')
                  when 'topbot', 'sides', 'none'; node.attr('frame')
                  else 'all'
                  end
      grid = node.attr('grid', 'all')
      rowsep = (grid == 'all' || grid == 'rows') ? 1 : 0
      colsep = (grid == 'all' || grid == 'cols') ? 1 : 0
      orient = node.attr?('orientation', 'landscape') ? 'land' : 'port'

      result = "<table#{attrs} frame=\"#{s1k_frame}\" pgwide=\"#{pgwide}\" rowsep=\"#{rowsep}\" colsep=\"#{colsep}\" orient=\"#{orient}\">"
      result << "<title>#{esc_text(node.title)}</title>" if node.title?
      result << "<tgroup cols=\"#{node.attr 'colcount'}\">"
      node.columns.each do |col|
        colname = "c#{col.attr 'colnumber'}"
        width_attr = col.attr?('width') ? %( colwidth="#{col.attr 'width'}*") : ""
        halign_attr = col.attr?('halign') ? %( align="#{col.attr 'halign'}") : ""
        result << "<colspec colname=\"#{colname}\"#{width_attr}#{halign_attr}/>"
      end

      [:head, :foot, :body].each do |tsec_name|
        rows = node.rows.send(tsec_name)
        next if rows.empty?
        result << "<t#{tsec_name}>"
        rows.each do |row|
          result << "<row>"
          row.each do |cell|
            entry_attrs = ""
            entry_attrs << %( align="#{cell.attr 'halign'}") if cell.attr? 'halign'
            entry_attrs << %( valign="#{cell.attr 'valign'}") if cell.attr? 'valign'
            col_num = cell.column.attr('colnumber')
            entry_attrs << %( namest="c#{col_num}" nameend="c#{col_num + cell.colspan - 1}") if cell.colspan.to_i > 1
            ...
            body = case cell.style
                   when :asciidoc; cell.content
                   when :literal; "<para><verbatimText>#{esc_text(cell.text)}</verbatimText></para>"
                   else cell.text.to_s.strip.empty? ? "<para/>" : "<para>#{esc_text(cell.text)}</para>"
                   end
            result << "<entry#{entry_attrs}>#{body}</entry>"
          end
          result << "</row>"
        end
        result << "</t#{tsec_name}>"
    ...
    def convert_image(node)
      attrs = common_attributes(node.id) + applic_ref_attribute(node)
      title = node.title ? "<title>#{esc_text(node.title)}</title>" : ""
      icn = node.attr('icn')
      unless icn && !icn.strip.empty?
        base = File.basename(node.attr('target', 'unknown.png'), '.*')
        if base.match?(/^ICN(-[A-Z0-9]{2,14}){2,5}-[A-Z0-9]{1,3}$/i)
          icn = base.upcase
          warn "asciidoctor: INFO: Inferred ICN '#{icn}' from image target. Use 'icn' attribute."
        else
          fallback = node.attr('alt') || base
          icn = "FIG-#{fallback.gsub(/[^A-Za-z0-9\-_\.]/, '_').gsub(/_+/, '_').upcase}"
          warn "asciidoctor: WARNING: No 'icn' for image. Using generated: #{icn}."
        end
      end
      "<figure#{attrs}>#{title}<graphic infoEntityIdent=\"#{esc_text(icn.strip)}\"/></figure>"
    end
    ...
    def convert_inline_quoted(node)
      ...
      if attrs && !attrs.empty? && open_tag.include?('>')
        parts = open_tag.rpartition('>')
        final_open = parts[1] == '>' ? "#{parts[0]}#{attrs}#{parts[1]}#{parts[2]}" : "#{open_tag.chomp}#{attrs}>"
      end
      "#{final_open}#{esc_text(node.text)}#{close_tag}"
    end

    def convert_admonition(node)
      attrs = common_attributes(node.id) + applic_ref_attribute(node)
      type = node.attr('name').upcase
      inner = if node.blocks?
                node.content
              else
                text = esc_text(node.source)
                case type
                when 'WARNING', 'CAUTION'; "<warningAndCautionPara>#{text}</warningAndCautionPara>"
                else "<notePara>#{text}</notePara>" # NOTE, TIP, IMPORTANT
                end
              end
      case type
      when 'WARNING'; "<warning#{attrs}>#{inner}</warning>"
      when 'CAUTION'; "<caution#{attrs}>#{inner}</caution>"
      when 'NOTE'; "<note#{attrs}>#{inner}</note>"
      when 'TIP', 'IMPORTANT'
        warn "asciidoctor: INFO: Admonition '#{type}' mapped to S1000D <note>."
        "<note#{attrs}>#{inner}</note>"
      else
        warn "asciidoctor: WARNING: Unknown admonition '#{type}' mapped to <note>."
        "<note#{attrs}>#{inner}</note>"
      end
    end
    ...
    def convert_inline_image(node)
      ...
      unless icn && !icn.strip.empty?
        ...
        if alt && alt.match?(/^(?:ICN|FIG)-/i)
          ...
          fallback = alt || base
          icn = "INL-FIG-#{fallback.gsub(/[^A-Za-z0-9\-_\.]/, '_').gsub(/_+/, '_').upcase}"
          warn "asciidoctor: INFO: Inline image missing 'icn'. Generated: #{icn}."
        end
      end
      "<graphic infoEntityIdent=\"#{esc_text(icn.strip)}\"/>"
    end

    def convert_fallback(node)
      warn %(asciidoctor: WARNING: S1000D converter unhandled node: '#{node.node_name}'. ID: '#{node.id}'. SKIPPED.)
      ""
    end
  end
end