...
module Asciidoctor
  class Converter::S1000D < Converter::Base
    ...
    def generate_main_procedure_steps_xml(document_node)
      ...
      blocks_to_process = if main_proc_section
      ...
      steps_content = blocks_to_process.each_with_index.map do |block, index|
        if block.context == :olist
          block.items.each_with_index.map do |li, li_idx|
            step_id = li.id || "#{block.id || 'procstep'}-#{li_idx + 1}"
            step_attrs = common_attributes(step_id) + applic_ref_attribute(li)
            inner_content = li.blocks? ? li.content : "<para#{applic_ref_attribute(li)}>#{esc_text(li.text)}</para>"
            "<proceduralStep#{step_attrs}>#{inner_content}</proceduralStep>"
          end.join("\n")
        else
          step_id = block.id || "procstep-#{index + 1}"
          step_attrs = common_attributes(step_id) + applic_ref_attribute(block)
          converted_content = block.convert
          converted_content.to_s.strip.empty? ? nil : "<proceduralStep#{step_attrs}>#{converted_content}</proceduralStep>"
        end
      end.compact.join("\n")
    ...
    def convert_olist(node)
      ...
      items = node.items.each_with_index.map do |item, idx|
        item_id = item.id || "#{node.id || 'seqlist'}-item-#{idx+1}"
        item_attrs = common_attributes(item_id) + applic_ref_attribute(item)
        inner = item.blocks? ? item.content : "<para#{applic_ref_attribute(item)}>#{esc_text(item.text)}</para>"
        "<listItem>#{inner}</listItem>"
      end.join
      "<para><sequentialList#{list_attrs}>#{items}</sequentialList></para>"
    end
