...
# It now handles both strict 11-part DMCs and condensed 8-part DMCs.
    ...
    def generate_preliminary_requirements_xml(document_node)
      ...
      if prelim_section
        prelim_section.blocks.each do |sub_block|
          ...
          title_key = "#{sub_block.id} #{sub_block.title}".downcase
          if title_key.include?('req_conds') || title_key.include?('required condition') || title_key.include?('required_conditions')
            content_map[:req_conds] = generate_req_cond_group_xml(sub_block)
          elsif title_key.include?('req_persons') || title_key.include?('required person') || title_key.include?('required_persons')
            content_map[:req_persons] = generate_req_persons_xml(sub_block)
          elsif title_key.include?('req_tech_info') || title_key.include?('required technical information') || title_key.include?('required_tech_info')
            content_map[:req_tech_info] = generate_req_tech_info_group_xml(sub_block)
          elsif title_key.include?('req_equip') || title_key.include?('support equipment') || title_key.include?('required_equip')
          ...
          elsif title_key.include?('req_consum') || title_key.include?('supply') || title_key.include?('required_supplies')
          ...
          elsif title_key.include?('req_spares') || title_key.include?('spare') || title_key.include?('required_spares')
          ...
          elsif title_key.include?('req_safety') || title_key.include?('safety condition') || title_key.include?('required safety') || title_key.include?('required_safety')
    ...
    def sched_list_item_texts(item)
      parts = []
      txt = item.text.to_s.strip
      parts << txt unless txt.empty?
      if item.blocks?
        item.blocks.each { |sub| parts.concat(sched_block_texts(sub)) }
      end
      parts
    end

    def sched_block_texts(block)
      parts = []
      case block.context
      when :paragraph
        txt = block.source.to_s.strip
        parts << txt unless txt.empty?
      when :ulist, :olist
        block.items.each { |item| parts.concat(sched_list_item_texts(item)) }
      when :section
        title_txt = block.title.to_s.strip
        parts << title_txt unless title_txt.empty?
        block.blocks.each { |sub| parts.concat(sched_block_texts(sub)) }
      when :open
        block.blocks.each { |sub| parts.concat(sched_block_texts(sub)) }
      end
      parts
    ...
    def collect_sched_tasks(document_node)
      tasks = []
      sched_section = document_node.blocks.find do |b|
        b.context == :section && (
          b.id == 'sched_tasks' ||
          b.title.downcase.include?('scheduled maintenance task') ||
          b.title.downcase.include?('maintenance task') ||
          b.title.downcase.include?('maintenance planning')
        )
      end

      candidate_blocks = sched_section ? sched_section.blocks : document_node.blocks
      section_tasks = candidate_blocks.select { |b| b.context == :section }

      if !section_tasks.empty?
        section_tasks.each do |sec|
          title_txt = sec.title.to_s.strip
          next if title_txt.empty?
          details = []
          sec.blocks.each { |blk| details.concat(sched_block_texts(blk)) }
          tasks << {
            title: title_txt,
            details: details.reject(&:empty?),
            applic_ref: sec.attr('applic_ref')
          }
        end
      else
        candidate_blocks.each do |blk|
          if blk.context == :olist
            blk.items.each do |item|
              txt = item.text.to_s.strip
              next if txt.empty?
              details = []
              if item.blocks?
                item.blocks.each { |sub| details.concat(sched_block_texts(sub)) }
              end
              tasks << {
                title: txt,
                details: details.reject(&:empty?),
                applic_ref: item.attr('applic_ref')
              }
            end
          end
        end
      ...
      if tasks.empty?
        tasks << { title: 'Perform scheduled maintenance task', details: [], applic_ref: nil }
      end
      tasks
    end

    def generate_sched_maint_xml(document_node)
      prelim = generate_preliminary_requirements_xml(document_node)
      tasks = collect_sched_tasks(document_node)

      task_defs = tasks.each_with_index.map do |task_hash, idx|
        task_id = format('%03d', idx + 1)
        title_txt = task_hash[:title].to_s.strip
        detail_paras = task_hash[:details].map { |d| "<simplePara>#{esc_text(d)}</simplePara>" }.join("\n")
        detail_paras = "<simplePara>#{esc_text(title_txt)}</simplePara>" if detail_paras.empty?
        task_applic_attr = task_hash[:applic_ref].to_s.strip.empty? ? "" : %( applicRefId="#{esc_text(task_hash[:applic_ref])}")
        <<~TASK_XML.strip
          <taskDefinition taskIdent="#{task_id}" worthinessLimit="recommended" reducedMaint="No"#{task_applic_attr}>
            <task>
              <taskDescr>
                <simplePara>#{esc_text(title_txt)}</simplePara>
                #{detail_paras}
              </taskDescr>
            </task>
            #{prelim.gsub(/^/, '    ')}
    ...
    def get_schema_file(dm_type_str)
      ...
      when 'crew';'crew.xsd';
      when 'sched','schedule','scheduled','scheduled maintenance','scheduledmaintenance','maintplanning';'schedul.xsd';
    ...
    def convert_document node
      ...
      content_blocks_for_general = node.blocks.reject do |block|; is_def_block = (block.role && ['applicdef','productdef','attribute_def','global_applicability_definition'].include?(block.role)) || (block.style && ['applicdef','productdef','attribute_def','global_applicability_definition'].include?(block.style)); is_part_of_proc_structure = false; 
        ...
        if ['sched','schedule','scheduled','scheduled maintenance','scheduledmaintenance','maintplanning'].include?(dm_type); 
          if block.context == :section; is_part_of_proc_structure ||= (block.id == 'prelim_reqs' || block.title.downcase.include?('preliminary req')); is_part_of_proc_structure ||= (block.id == 'sched_tasks' || block.title.downcase.include?('scheduled maintenance task') || block.title.downcase.include?('maintenance task') || block.title.downcase.include?('maintenance planning')); end;
        end;
      ...
      main_dm_content = case dm_type
                        ...
                        when 'fault', 'faultisolation'; 
                          ...
                          FAULT_XML
                        when 'sched','schedule','scheduled','scheduled maintenance','scheduledmaintenance','maintplanning'
                          generate_sched_maint_xml(node)
                        when 'act'; 
    ...
    def convert_ulist(node, nested: false)
      ...
      begin
        ...
        item_outputs = node.items.map.with_index do |item, idx|
          ...
          if item.blocks?
            item.blocks.each { |blk| nested_content << blk.convert }
          end

          inner_content = processed_text.empty? && nested_content.empty? ? "<para/>" : "<para>#{processed_text}#{nested_content}</para>"
...
def convert_section(node)
  ...
  when :ref, :bibref
        ...
        rows_collection.each do |row|
          ...
          row.each do |cell|
           ...
           when :asciidoc
             cell.content
           when :literal
             # Remove esc_text here as well if it causes double escaping
             "<para><verbatimText>#{cell.text}</verbatimText></para>"
           else
             # REMOVE esc_text(cell.text) and just use cell.text
             cell.text.to_s.strip.empty? ? "<para/>" : "<para>#{cell.text}</para>"
           end
    ...
    def convert_inline_image(node)
      ...
      # Determine the ICN value (reusing your existing logic)
      unless icn_val && !icn_val.strip.empty?
        ...
        if alt_text && alt_text.match?(/^(?:ICN|FIG)-/i)
          ...
          # Logic specific to your requirement: if it's an icon, prefix it correctly if needed
          prefix = (target_path.include?('icons/') || role == 'symbol') ? "ICN" : "INL-FIG"
          icn_val = "#{prefix}-#{fallback_base.gsub(/[^A-Za-z0-9\-_\.]/, '_').gsub(/_+/, '_').upcase}"
        end
      end

      # Logic for S1000D <symbol>
      # We trigger this if the path contains 'icons/' or the user explicitly sets [role=symbol]
      if target_path.include?('icons/') || node.attr('role') == 'symbol'
    scale = node.attr('width') || "60"
    # Return raw string - Asciidoctor will preserve this if parent uses apply_subs
    return %(<symbol infoEntityIdent="#{esc_text(icn_val.strip)}" reproductionScale="#{esc_text(scale)}" />)
  end
