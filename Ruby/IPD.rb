    ...
    def generate_ipd_content_xml(node)
      ...
      if figure_section
        ...
        graphics_xml = figure_section.blocks.select { |b| b.context == :image }.map do |img_block|
          ...
          # Prefer a nearby table with role 'hotspots' or attribute 'hotspots-for', otherwise fall back to an image 'hotspots' attribute
          hotspots_table = nil
          idx = figure_section.blocks.index(img_block)
          following = (idx && idx < figure_section.blocks.size - 1) ? figure_section.blocks[(idx + 1)..-1] : []
          following.each do |b|
            break if b.context == :image
            if b.context == :table
              if b.role == 'hotspots' || b.attr('hotspots-for') || (b.title && b.title.downcase.include?('hotspot'))
                hotspots_table = b
                break
              end
            end
          ...
          hotspots_attr = img_block.attr('hotspots')
          if hotspots_table || (hotspots_attr && !hotspots_attr.to_s.strip.empty?)
            hotspots_xml = []

            # If a hotspots table exists, parse it (preferred)
            if hotspots_table
              begin
                header = nil
                if hotspots_table.rows.respond_to?(:head) && hotspots_table.rows.head && !hotspots_table.rows.head.empty?
                  header = hotspots_table.rows.head[0].map { |c| c.respond_to?(:text) ? c.text.to_s.strip.downcase : c.to_s.strip.downcase }
                end
                hotspots_table.rows.body.each_with_index do |row, ridx|
                  cells = row.map { |c| c.respond_to?(:text) ? c.text.to_s.strip : c.to_s.strip }
                  if header && header.any?
                    hash = {}
                    header.each_with_index { |h, i2| hash[h] = cells[i2] || '' }
                    hid = hash['id'] || hash['ident'] || hash['hotspotid'] || "#{img_block.id || 'fig'}-hot-#{ridx}"
                    app_id = esc_text(hash['appid'] || hash['applicationstructureident'] || hash['app'] || hash['applicationstructureid'] || "hot#{ridx}")
                    app_name = esc_text(hash['name'] || hash['applicationstructurename'] || hash['applicationstructurename'] || hash['appname'] || ridx.to_s)
                    title = esc_text(hash['title'] || hash['hotspottitle'] || '')
                    type = esc_text(hash['type'] || hash['hotspottype'] || 'CALLOUT')
                    descr = esc_text(hash['descr'] || hash['objectdescr'] || '')
                  else
                    # positional: id, appId, name, title, type, descr
                    hid = cells[0] && !cells[0].empty? ? cells[0] : "#{img_block.id || 'fig'}-hot-#{ridx}"
                    app_id = esc_text(cells[1] || "hot#{ridx}")
                    app_name = esc_text(cells[2] || ridx.to_s)
                    title = esc_text(cells[3] || '')
                    type = esc_text(cells[4] || 'CALLOUT')
                    descr = esc_text(cells[5] || '')
                  end
                  hotspots_xml << "      <hotspot id=\"#{esc_text(hid)}\" applicationStructureIdent=\"#{app_id}\" applicationStructureName=\"#{app_name}\" hotspotType=\"#{type}\" hotspotTitle=\"#{title}\" objectDescr=\"#{descr}\"/>"
                end
              rescue => e
                warn "asciidoctor: WARNING: Failed to parse hotspots table: #{e.message}"
              end
            else
              # No table; fallback to hotspots attribute (JSON or CSV)
              begin
                parsed = JSON.parse(hotspots_attr) rescue nil
                if parsed.is_a?(Hash)
                  parsed.each do |key, h|
                    h = {} unless h.is_a?(Hash)
                    hid = h['id'] || h['Id'] || "#{img_block.id || 'fig'}-#{key}"
                    app_id = esc_text(h['applicationStructureIdent'] || h['appId'] || key)
                    app_name = esc_text(h['applicationStructureName'] || h['applicationStructureName'] || h['name'] || '')
                    title = esc_text(h['hotspotTitle'] || h['title'] || '')
                    type = esc_text(h['hotspotType'] || h['type'] || 'CALLOUT')
                    descr = esc_text(h['objectDescr'] || h['descr'] || '')
                    hotspots_xml << "      <hotspot id=\"#{esc_text(hid)}\" applicationStructureIdent=\"#{app_id}\" applicationStructureName=\"#{app_name}\" hotspotType=\"#{type}\" hotspotTitle=\"#{title}\" objectDescr=\"#{descr}\"/>"
                  end
                elsif parsed.is_a?(Array)
                  parsed.each_with_index do |h, idx|
                    h = {} unless h.is_a?(Hash)
                    hid = h['id'] || "#{img_block.id || 'fig'}-hot-#{idx}"
                    app_id = esc_text(h['applicationStructureIdent'] || h['appId'] || "hot#{idx}")
                    app_name = esc_text(h['applicationStructureName'] || h['name'] || idx.to_s)
                    title = esc_text(h['hotspotTitle'] || h['title'] || '')
                    type = esc_text(h['hotspotType'] || h['type'] || 'CALLOUT')
                    descr = esc_text(h['objectDescr'] || h['descr'] || '')
                    hotspots_xml << "      <hotspot id=\"#{esc_text(hid)}\" applicationStructureIdent=\"#{app_id}\" applicationStructureName=\"#{app_name}\" hotspotType=\"#{type}\" hotspotTitle=\"#{title}\" objectDescr=\"#{descr}\"/>"
                ...
                warn "asciidoctor: WARNING: Failed to parse hotspots JSON: #{e.message}"
              end

              if hotspots_xml.empty?
                hotspots_attr.to_s.split(';').each_with_index do |rec, idx|
                  fields = rec.split('|').map(&:strip)
                  app_id = esc_text(fields[0] || "hot#{idx}")
                  app_name = esc_text(fields[1] || idx.to_s)
                  title = esc_text(fields[2] || '')
                  type = esc_text(fields[3] || 'CALLOUT')
                  descr = esc_text(fields[4] || '')
                  hid = "#{img_block.id || 'fig'}-hot-#{idx}"
                  hotspots_xml << "      <hotspot id=\"#{esc_text(hid)}\" applicationStructureIdent=\"#{app_id}\" applicationStructureName=\"#{app_name}\" hotspotType=\"#{type}\" hotspotTitle=\"#{title}\" objectDescr=\"#{descr}\"/>"
                end
              end
            end
...
def convert_section(node)
  ...
  when :ref, :bibref
    ...
    def convert_inline_image(node)
      ...
      if target_path.include?('icons/') || role == 'symbol'
        # reproductionScale: use the width attribute if provided (e.g., the '16' in [Warning,16,16])
        # In S1000D, scale is often expressed as an integer. If no width, default to 60.
        scale = node.attr('width') || "60"
        
        return %(<symbol infoEntityIdent="#{esc_text(icn_val.strip)}" reproductionScale="#{esc_text(scale)}" />)
      end

      # Fallback for standard inline graphics
      %(<graphic infoEntityIdent="#{esc_text(icn_val.strip)}"/>)
    end
