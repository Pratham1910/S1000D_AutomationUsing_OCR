...
module Asciidoctor
  class Converter::S1000D < Converter::Base
    ...
    # ############# START: UPDATED REQ COND METHOD (Removes externalRef) #############
    def generate_req_cond_group_xml(section_node, for_closeout = false)
      ...
      section_node.blocks.each_with_index do |block, idx|
        case block.context
        when :paragraph
          # Existing logic to skip cross-reference target paragraphs
          ...
          unless is_likely_xref_target_only
            warn "asciidoctor: WARNING: Paragraph found directly in '#{section_node.title}' section. SKIPPED."
          end
        when :ulist, :olist
          ...
          block.items.each do |item|
            ...
            # Attributes for explicit references
            ...
            # 1. Get raw text to search for codes (DMC/PMC)
            # Use .source if available to see the raw "link:..." macro, otherwise text
            raw_source_text = item.respond_to?(:source) ? item.source : item.text.to_s
            text_for_condition = raw_source_text.dup.strip
            ...
            if pm_ref_attr && !pm_ref_attr.strip.empty?
              ...
              # Regex to find DMC/PMC in the text or link target
            ...
            # 3. Clean up attribute text from the condition text
            if text_for_condition.match?(/\A\s*\[id=([^,\]\s]+)/i); text_for_condition.sub!(/\A\s*\[id=[^\]]+\]\s*/i, '').strip!; end
            
            # 4. Generate the inner XML (This usually creates <externalRef> for links)
            req_cond_inner_xml = if item.blocks?
                                   item.content 
                                 else
                                   convert_text_with_inline_xrefs(item.text.strip, item)
                                 end

            # 5. Build the XML Element
            if pm_to_parse && !pm_to_parse.strip.empty?
              parsed_pm_ref_code = parse_pmc_string(pm_to_parse)
              if parsed_pm_ref_code
                # Strip externalRef tags if we are wrapping in a PM reference
                clean_cond_text = req_cond_inner_xml.gsub(/<externalRef[^>]*>(.*?)<\/externalRef>/m, '\1')
                
                pm_ref_xml_part = %(<pmRef><pmRefIdent><pmCode modelIdentCode="#{esc_text(parsed_pm_ref_code[:modelIdentCode])}" pmIssuer="#{esc_text(parsed_pm_ref_code[:pmIssuer])}" pmNumber="#{esc_text(parsed_pm_ref_code[:pmNumber])}" pmVolume="#{esc_text(parsed_pm_ref_code[:pmVolume])}"/><language languageIsoCode="#{esc_text(parsed_pm_ref_code[:languageIsoCode])}" countryIsoCode="#{esc_text(parsed_pm_ref_code[:countryIsoCode])}"/></pmRefIdent></pmRef>)
                conditions_xml_elements << "<reqCondPm#{item_s1000d_id_attr_str}#{item_s1000d_applic_attr_str}><reqCond>#{clean_cond_text}</reqCond>#{pm_ref_xml_part}</reqCondPm>"
              else 
              ...
              if parsed_dm_ref_code
                ...
                # === FIX IS HERE ===
                # If we detected a DM code, strip the <externalRef> tags created by the link macro.
                # Regex matches: <externalRef ...>CONTENT</externalRef> and replaces it with CONTENT
                clean_cond_text = req_cond_inner_xml.gsub(/<externalRef[^>]*>(.*?)<\/externalRef>/m, '\1')
                
                conditions_xml_elements << "<reqCondDm#{item_s1000d_id_attr_str}#{item_s1000d_applic_attr_str}><reqCond>#{clean_cond_text}</reqCond>#{dm_ref_xml_part}</reqCondDm>"
              else 
                conditions_xml_elements << "<reqCondNoRef#{item_s1000d_id_attr_str}#{item_s1000d_applic_attr_str}><reqCond>#{req_cond_inner_xml}</reqCond></reqCondNoRef>"
              end
            else
              # Standard condition (no reference)
              conditions_xml_elements << "<reqCondNoRef#{item_s1000d_id_attr_str}#{item_s1000d_applic_attr_str}><reqCond>#{req_cond_inner_xml}</reqCond></reqCondNoRef>"
            end
          end
        when :admonition
    ...
    # ############# START: SECTION-BASED FAULT METHOD #############
    # ############# START: UPDATED FAULT METHOD (Text Only - No Para Tags) #############
    def generate_fault_isolation_main_procedure_xml(document_node)
      ...
      blocks_to_process.each do |block|
        ...
        if block.role == 'faultstep'
          ...
          # Split Action vs Question (Last paragraph is Question)
          ...
          # Helper to extract raw text without <para> tags
          extract_content = ->(blks) {
            blks.map do |b| 
              if b.context == :paragraph
                b.content # Raw text only
              else
                b.convert # Keep lists/tables if they exist, as they have their own tags
              end
            end.join("\n").strip
          }
        ...
        # 3. Logic for FAULT END
        elsif block.role == 'faultend'
          # Helper to extract raw text
          extract_content = ->(blks) {
            blks.map { |b| (b.context == :paragraph) ? b.content : b.convert }.join("\n").strip
          }
          action_content = extract_content.call(block.blocks)
      ...
      if isolation_steps_content.strip.empty?
        isolation_steps_content = "<isolationProcedureEnd id=\"auto-end-no-steps-found\"><action>No fault isolation steps provided.</action></isolationProcedureEnd>"
      end
    ...
    # ############# END: UPDATED FAULT METHOD (Text Only) #############
